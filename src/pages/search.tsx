...
import {emptyCatalog, getCatalogData} from "@/services/traveloop-api"

export function SearchPage() {
  const {data, error, isLoading} = useSupabaseQuery("catalog-search", emptyCatalog, getCatalogData)
  const results = [
  ...
  return (
    <PageShell>
      ...
      {error ? <div className="mt-4"><ErrorState description={error} title="Could not load search catalog" /></div> : null}
      ...
      <section className="mt-5 rounded-xl border border-border bg-card p-4 shadow-soft">
        ...
        {isLoading ? (
            ...
            {results.map((result) => (
              <Card className="overflow-hidden" key={`${result.kind}-${result.id}`}>
                <CardContent className="grid gap-4 pt-5 sm:grid-cols-[96px_1fr_auto] sm:items-center">
                  <img alt={`${result.kind}: ${result.title}`} className="h-24 w-full rounded-lg object-cover sm:w-24" src={result.image} />
                  <div>
          ...
          <EmptyState
            description="Run the India market seed migration or add catalog rows in Supabase to populate search."
            title="No city or activity results yet"
          />
        )}
      </section>
    </PageShell>
